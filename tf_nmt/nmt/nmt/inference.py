...
def single_worker_inference(sess,
  ...
  #print("THE INFER DATA IS: ", infer_data) # DELETE
  user_string = "" # DELETE
  while not user_string.strip(): # DELETE
    user_string = input("NHẬP CÂU CẦN DỊCH: ") # DELETE
  replace_infer_data = [user_string] # DELETE
  #print("THE REPLACE INFER DATA IS: ", replace_infer_data) # DELETE
  infer_data = replace_infer_data # DELETE
  #print("THE NEW INFER DATA IS: ", infer_data) # DELETE
  ...
  with infer_model.graph.as_default():
    ...
    # Decode
    #utils.print_out("# Start decoding") # REMOVE COMMENT
    if hparams.inference_indices:
      ...
      print("KẾT QUẢ DỊCH LÀ:", ''.join(load_data(output_infer, hparams))) # DELETE
